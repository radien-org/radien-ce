...
public class PageMessageBodyReaderTest {
    ...
    public void testReadFrom() throws IOException {
        ...
        Page<${entityResourceName}> page = target.readFrom(null,null,null,null,null,in);
        assertEquals(1,page.getTotalPages());
        assertEquals(4,page.getTotalResults());
        assertEquals(-1,page.getCurrentPage());
        assertEquals(1,page.getResults().size());
        ${entityResourceName} ${entityResourceName.toLowerCase()} = page.getResults().get(0);
        assertEquals("a",${entityResourceName.toLowerCase()}.getName());
        assertEquals(28L,(long) ${entityResourceName.toLowerCase()}.getId());
    }
}
