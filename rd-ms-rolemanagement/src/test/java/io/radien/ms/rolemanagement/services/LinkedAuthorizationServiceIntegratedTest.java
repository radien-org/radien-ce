...
//    }
    ...
    public void testIntegrationAmongEntities() throws RoleNotFoundException, UniquenessConstraintException, LinkedAuthorizationNotFoundException {
        ...
        Long permissionIdForUserAdminRole = 1002L;
        ...
        boolean exists = linkedAuthorizationServiceAccess.existsRole(idUser1, null,
                main.getName());
        assertTrue(exists);

        Long tenantId = 1000L;
        exists = linkedAuthorizationServiceAccess.existsRole(idUser1, tenantId,
                main.getName());
        assertFalse(exists);

        exists = linkedAuthorizationServiceAccess.existsRole(idUser2, tenantId,
                main.getName());
        assertFalse(exists);

        exists = linkedAuthorizationServiceAccess.existsRole(idUser3, tenantId,
                main.getName());
        assertFalse(exists);

        // Checking for "Tenant Admin"
        exists = linkedAuthorizationServiceAccess.existsRole(idUser1, null,
                tenantAdmin.getName());
        assertTrue(exists);

        exists = linkedAuthorizationServiceAccess.existsRole(idUser1, 1111L,
                tenantAdmin.getName());
        assertTrue(exists);
    }
