...
public class RoleService implements RoleServiceAccess {
    private static final long serialVersionUID = 5369712139004673096L;
     ...
     */
    @Override
    public SystemRole get(Long roleId) {
     ...
     */
    @Override
    public void create(SystemRole role) throws UniquenessConstraintException, InvalidArgumentException {
        ValidationUtil.checkIfMandatoryParametersWereInformed(role.getName());
     ...
     */
    @Override
    public void update(SystemRole role) throws UniquenessConstraintException, InvalidArgumentException {
        ValidationUtil.checkIfMandatoryParametersWereInformed(role.getId(), role.getName(), role.getDescription());
        List<RoleEntity> alreadyExistentRecords = searchDuplicatedName(role);
