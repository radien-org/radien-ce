 ...
 */
@Stateless
public class KeycloakBusinessService {
    protected static final Logger log = LoggerFactory.getLogger(KeycloakBusinessService.class);
     ...
     * @return the newly created user subject
     ...
     * @throws RemoteResourceException exceptions that may occur during the execution of a remote method call.
     */
    public void updateUser(SystemUser newUser) {
     ...
     * @throws RemoteResourceException exceptions that may occur during the execution of a remote method call.
     */
    public void sendUpdatePasswordEmail(SystemUser user) {
        KeycloakClient client = keycloakClientFactory.getKeycloakClient();
     ...
     * @throws RemoteResourceException exceptions that may occur during the execution of a remote method call.
     */
    public void updateEmailAndExecuteActionEmailVerify(String email, String sub, boolean emailVerify) {
     ...
     * @throws BadRequestException thrown in case of invalid credentials
     * @throws RemoteResourceException thrown in case of any issue regarding communication with KeyCloak service
     */
    public void validateChangeCredentials(String username, String subject, String password, String newPassword) {
        KeycloakClient client = keycloakClientFactory.getKeycloakClient();
        if (!client.validateCredentials(username, password)) {
            throw new BadRequestException(GenericErrorCodeMessage.ERROR_INVALID_CREDENTIALS.toString());
        }
