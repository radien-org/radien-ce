...
public class ResourceService implements ResourceServiceAccess {
    ...
    public Page<SystemResource> getAll(String search, int pageNo, int pageSize,
        ...
        return new Page<>(systemResources, pageNo, totalRecords, totalPages);
    }
     ...
     */
    @Override
    public List<SystemResource> getResources(SystemResourceSearchFilter filter) {
        EntityManager em = getEntityManager();
        CriteriaBuilder criteriaBuilder = em.getCriteriaBuilder();
        CriteriaQuery<ResourceEntity> criteriaQuery = criteriaBuilder.createQuery(ResourceEntity.class);
        Root<ResourceEntity> resourceRoot = criteriaQuery.from(ResourceEntity.class);
     ...
     */
    private void checkUniqueness(SystemResource resource) throws UniquenessConstraintException {
        List<ResourceEntity> alreadyExistentRecords = searchDuplicatedName(resource);
     ...
     */
    private List<ResourceEntity> searchDuplicatedName(SystemResource resource) {
        EntityManager em = getEntityManager();
    ...
    private Predicate getFilteredPredicate(ResourceSearchFilter filter,
                                           CriteriaBuilder criteriaBuilder, Root<ResourceEntity> resourceRoot) {
        Predicate global;
     ...
     * @return a constructed predicate with the fields needed to be search
     */
    private Predicate getFieldPredicate(String name, Object value, ResourceSearchFilter filter, 
                                        CriteriaBuilder criteriaBuilder, Root<ResourceEntity> resourceRoot, Predicate global) {
        if(value != null) {
