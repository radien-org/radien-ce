...
const ExpandableSection = dynamic(
    () => import("@cloudscape-design/components/expandable-section"),
    { ssr: false}
);

export default function TenantRequestsTable() {
    const { i18n, activeTenant: { data: activeTenantData }  } = useContext(RadienContext);
    ...
    const getTenantRequestsPage = async (pageNumber: number = 1, pageSize: number = 10) => {
        const result = await axios.get("/api/ticket/getTenantRequests", {
            params: {
                ticketType: TicketType.TENANT_REQUEST,
                data: activeTenantData?.tenantId,
                page: pageNumber,
                pageSize: pageSize
            }
        });
        setRequestCount(result.data?.totalResults);
        return result;
    }
        ...
        <Box padding={"xl"}>
            ...
            <ExpandableSection
                ...
                <PaginatedTable
                    ...
                    getPaginated={getTenantRequestsPage}
                    viewActionProps={{}}
                    createActionProps={{
                    ...
                    deleteActionProps={
                        {
                            ...
                            deleteConfirmationText: (selectedUser) => `${i18n?.user_management_tenant_request_reject_action_confirmation || "Are you sure you would like to reject ${}"}`
                                .replace("${}", `${selectedUser?.user.firstname} ${selectedUser?.user.lastname}`),
