...
export default function TenantRequestModal(props: TenantRequestProps) {
    ...
    const notifyAdmins = (option: OptionDefinition, referenceUrl: string) => {
        ...
        notifyTenantRoles
            ...
            .then(
                (r) => {
                    const message = `${i18n?.generic_success_message || "Success"}: ${
                        i18n.tenant_request_admins_notified || "Administrators of ${} have been notified"
                ...
                (e) => {
                    if (e instanceof AxiosError && e.response?.status == 404) {
                        const message = `${i18n?.generic_error_message || "Error"}: ${
                            i18n.tenant_request_no_admins_found || "No users with the necessary roles were found in ${}"
                        }`.replace("${}", option.label!);
    ...
    const onClickAction = () => {
        ...
        selectedOptions.forEach((option) => {
            ...
            createTicket.mutateAsync(ticket).then(
                (r) => {
                    notifyAdmins(option, referenceUrl);
                },
                (e) => {
                    const message = `${i18n?.generic_error_message || "Error"}: ${
                        i18n.tenant_request_create_ticket_error || "Could not create Tenant Request. If the error persists please contact an administrator."
                    }`;
                    addErrorMessage(message);
                    popLoadingState();
                }
    ...
    return (
        <Modal
                ...
                <Box float="right">
                    <SpaceBetween direction="horizontal" size="xs">
                        <Button variant="primary" onClick={() => onClickAction()}>
                            {i18n?.tenant_request_button_primary || "Ok"}
                        </Button>
            ...
            header={i18n?.tenant_request_header || "Request tenant"}>
            <Multiselect
                ...
                placeholder={i18n?.tenant_request_options_placeholder || "Choose tenant(s)"}
            />
        </Modal>
    );
}
